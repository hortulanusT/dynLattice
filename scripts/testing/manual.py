...
for entry in os.scandir("tests/manual"):
  if entry.is_dir() and entry.name == "testing":
    ...
    print(states.index.get_level_values("time").max())
    # states = states.loc[.001:.002]
    ...
    with PdfPages(os.path.join(entry, "plots.pdf")) as file:
      for node in states.columns.get_level_values("node").unique():
        for state in range(3):
          print(f"\tnode{node}\t{statename[state]}")
          states.xs(state, level="state").xs(node, axis="columns", level="node").plot.line( subplots=True, layout=[2,3], ls='', marker='.', figsize=(10,6), title=f"{study_type} -- {statename[state]} node{node}" )
