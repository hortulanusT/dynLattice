...
def bending_u(d): return 1-(math.sin(d)/d)
def bending_v(d): return (1-math.cos(d))/d
...
ideal_disp["x"]["dy"] = np.array(
    [[math.nan, math.nan, math.nan, math.nan, math.nan, math.nan] for _ in displs])
...
for lDir in load_dir:
  for lTyp in load_typ:
    ...
    disp_comp = np.allclose(
        sim_disp, ideal_disp[elem_dir][lDir], atol=1e-6, rtol=1e-3)
    ...
    if "Diff" in test_runs[lTyp+"_"+lDir] or "???" == test_runs[lTyp+"_"+lDir]:
      np.savetxt(
          f"tests/element/runs/{sys.argv[1]}_{lTyp}_{lDir}-disp-ana.csv", ideal_disp[elem_dir][lDir], fmt="%16.8e")
      np.savetxt(
          f"tests/element/runs/{sys.argv[1]}_{lTyp}_{lDir}-resp-ana.csv", ideal_resp[lDir], fmt="%16.8e")
...
if all(outcome == "analytical" or outcome == "???" for outcome in test_runs.values()):
  ...
  print(colored(
      f"Some tests for {sys.argv[1]:8} have different displacements", "yellow"))
else:
  print(
      colored(f" !!! Tests for {sys.argv[1]:8} failed", "red", attrs=["bold"]))
