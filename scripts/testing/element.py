...
def bending_u(d):
  return 1 - (math.sin(d) / d)


def bending_v(d):
  return (1 - math.cos(d)) / d
...
ideal_disp["x"]["dy"] = np.array(
    [[math.nan, math.nan, math.nan, math.nan, math.nan, math.nan]
     for _ in displs])
ideal_disp["x"]["dz"] = np.array(
    [[math.nan, math.nan, math.nan, math.nan, math.nan, math.nan]
     for _ in displs])
ideal_disp["x"]["rx"] = np.array([[0, 0, 0, d, 0, 0] for d in displs])
ideal_disp["x"]["ry"] = np.array(
    [[-bending_u(d), 0, -bending_v(d), 0, d, 0] for d in displs])
ideal_disp["x"]["rz"] = np.array(
    [[-bending_u(d), bending_v(d), 0, 0, 0, d] for d in displs])

ideal_disp["y"]["dx"] = np.array(
    [[math.nan, math.nan, math.nan, math.nan, math.nan, math.nan]
     for _ in displs])
ideal_disp["y"]["dy"] = np.array([[0, d, 0, 0, 0, 0] for d in displs])
ideal_disp["y"]["dz"] = np.array(
    [[math.nan, math.nan, math.nan, math.nan, math.nan, math.nan]
     for _ in displs])
ideal_disp["y"]["rx"] = np.array(
    [[0, -bending_u(d), bending_v(d), d, 0, 0] for d in displs])
ideal_disp["y"]["ry"] = np.array([[0, 0, 0, 0, d, 0] for d in displs])
ideal_disp["y"]["rz"] = np.array(
    [[-bending_v(d), -bending_u(d), 0, 0, 0, d] for d in displs])

ideal_disp["z"]["dx"] = np.array(
    [[math.nan, math.nan, math.nan, math.nan, math.nan, math.nan]
     for _ in displs])
ideal_disp["z"]["dy"] = np.array(
    [[math.nan, math.nan, math.nan, math.nan, math.nan, math.nan]
     for _ in displs])
ideal_disp["z"]["dz"] = np.array([[0, 0, d, 0, 0, 0] for d in displs])
ideal_disp["z"]["rx"] = np.array(
    [[0, -bending_v(d), -bending_u(d), d, 0, 0] for d in displs])
ideal_disp["z"]["ry"] = np.array(
    [[bending_v(d), 0, -bending_u(d), 0, d, 0] for d in displs])
...
for lDir in load_dir:
  for lTyp in load_typ:
    ...
    try:
      sim_disp = np.loadtxt(
          f"tests/element/runs/{sys.argv[1]}_{lTyp}_{lDir}-disp.csv")
      sim_resp = np.loadtxt(
          f"tests/element/runs/{sys.argv[1]}_{lTyp}_{lDir}-resp.csv")
    except:
      test_runs[lTyp + "_" + lDir] = ">< FAIL ><"
      continue

    disp_comp = np.allclose(sim_disp,
                            ideal_disp[elem_dir][lDir],
                            atol=1e-6,
                            rtol=1e-3)
    load_comp = np.allclose(sim_resp,
                            ideal_resp[lDir],
                            atol=1e-6,
                            rtol=1e-3)
    ...
    elif np.isnan(ideal_disp[elem_dir][lDir]).all(
    ) and lDir[-1] != elem_dir and lDir[0] == "d":
    ...
    if "Diff" in test_runs[lTyp + "_" +
                           lDir] or "???" == test_runs[lTyp + "_" +
                                                       lDir]:
      np.savetxt(
          f"tests/element/runs/{sys.argv[1]}_{lTyp}_{lDir}-disp-ana.csv",
          ideal_disp[elem_dir][lDir],
          fmt="%16.8e")
      np.savetxt(
          f"tests/element/runs/{sys.argv[1]}_{lTyp}_{lDir}-resp-ana.csv",
          ideal_resp[lDir],
          fmt="%16.8e")

# finishing
with open(f"tests/element/result_{sys.argv[1]}.txt",
          "w") as result_file:
...
if all(outcome == "analytical" or outcome == "???"
       for outcome in test_runs.values()):
  print(colored(f" All tests for {sys.argv[1]:8} passed", "green"))
elif not any(outcome == "_loadDiff_" or outcome == "_ALL_Diff_"
             for outcome in test_runs.values()):
  print(
      colored(
          f"Some tests for {sys.argv[1]:8} have different displacements",
          "yellow"))
else:
  print(
      colored(f" !!! Tests for {sys.argv[1]:8} failed",
              "red",
              attrs=["bold"]))
