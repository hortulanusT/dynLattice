...

{
  ...
  if (action == Actions::GET_MATRIX0)
  {
    ...
    // the internal vector.
    if (rotDofs_.size() > 0)
      assembleRot_(*mbld, fint, disp);
    if (transDofs_.size() > 0)
      assembleTrans_(*mbld, fint, disp);
...
void rodJointModel::init_()
{
  ...
  for (idx_t iDof = 0; iDof < rotDofs_.size(); iDof++)
    iRotDofs_[iDof] = dofs_->getTypeIndex(rotDofs_[iDof]);
  for (idx_t iDof = 0; iDof < transDofs_.size(); iDof++)
    iTransDofs_[iDof] = dofs_->getTypeIndex(transDofs_[iDof]);
}
