    ...
    : Shape(name)
{
  ...
  {
  case 2:
    stdline = newInstance<StdLine2>();
    break;
  case 3:
    stdline = newInstance<StdLine3>();
    break;
  case 4:
    stdline = newInstance<StdLine4>();
    break;
  default:
    throw jem::IllegalInputException(getContext(), "given number of nodes not supported!");
  }
                   ...
                   const Matrix &c) const
{
  ...
  for (idx_t ip = 0; ip < ipointCount(); ip++)
    ...
    {
      for (idx_t i = 0; i < 6; i++)
        Xi(i, i, iNode, ip) = grads(iNode, ip);
                    ...
                    const Matrix &c) const
{
  ...
  for (idx_t ip = 0; ip < ipointCount(); ip++)
  {
    ...
    {
      for (idx_t i = 0; i < 6; i++)
        Psi(i, i, iNode, ip) = grads(iNode, ip);
      for (idx_t i = 0; i < 3; i++)
        Psi(i + 3, i + 6, iNode, ip) = shapes(iNode, ip);
    }
  }
}
...
//------------------------------------------------------
void Line3D::getRefRot_(const Matrix Lambda_r,
                        const Cubix Rn) const
{
...
void Line3D::getNodeRotVecs_(const Matrix psi,
