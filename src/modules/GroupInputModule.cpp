...

{
  ...
  else
  {
    ...
    for (idx_t igr = 0; igr < restrictTo_.size(); ++igr)
    {
      ...
      ElementGroup egroup(ElementGroup::get(restrictTo_[igr], elems, globdat, "NGroup_::findCandidates"));
...

{
  ...
  System::info() << " ...Created NodeGroup `" << myName_ << "' with " << inodes_.size() << " nodes.\n";
  for (idx_t inode = 0; inode < inodes_.size(); inode++) // TG
  {
    Vector coords(rank_);
    nodes.getNodeCoords(coords, inodes_[inode]);
    System::debug() << "\tNode No. " << inodes_[inode] << " with coords: " << coords << "\n";
  }
...

{
  ...
  else if (testany(doX_))
  {
    ...
    {
      ElementGroup egroup(ElementGroup::get(parent_, elems, globdat,
...

{
  ...
  System::info() << " ...Created ElementGroup `" << myName_ << "' with " << ielems_.size() << " elements.\n";
