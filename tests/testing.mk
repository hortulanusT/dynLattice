...
.PRECIOUS: tests/beam/test%/disp.csv tests/beam/test%/resp.csv tests/element/runs/%.log
...
tests/element/runs/%.log: $(program) tests/element/test.pro
	@mkdir -p $(dir $@)
	...
	@$^ -p Input.input.order=$(order)\
			...
			-p Output.disp.file=\"$(addsuffix -disp.res, $(basename $@))\"\
			-p Output.resp.file=\"$(addsuffix -resp.res, $(basename $@))\"\
			-p Output.load.file=\"$(addsuffix -load.res, $(basename $@))\"\
			> $@
