...
data = pd.read_csv("tests/transient/test2_plastic/disp.gz",
...
plt.legend()
plt.tight_layout()
plt.savefig("tests/transient/test2_plastic/result.pdf")

if max(data.index) > 0.9:
  print(colored("TRANSIENT TEST 2 (PLASTIC) RUN THROUGH", "green"))
else:
  print(colored("TRANSIENT TEST 2 (PLASTIC) FAILED", "red", attrs=["bold"]))
