...
YIELD="abs(dx/(0.1 * 280/450*  820-h_dx))^2.68 + abs(dy/(0.1 * 280/450*  820-h_dy))^2.68 + abs(dz/(0.1 * 280/450* 1870-h_dz))^1.75 + abs(rx/(0.1 * 280/450* 0.94-h_rx))^1.93 + abs(ry/(0.1 * 280/450* 0.94-h_ry))^1.93 + abs(rz/(0.1 * 280/450* 0.75-h_rz))^1.70 - 1";
...
./bin/nonlinRod studies/output/tno-design/ARH/evo/uniaxial-1x1.pro \
  -p "log.pattern='*.info'" -p "control.runWhile='ymax.disp.dy/SIZE.Y>-0.005'" \
  -p "Output.tangent.sampleWhen='(i-1)%10<1'" -p "Output.sampling.sampling.sampleWhen='(i-1)%10<1'"  \
  -p "model.model.lattice.child.material.yieldCond='$YIELD'" -p "model.model.lattice.child.material.precision=0." > /dev/null &
...
#   -p "Output.tangent.sampleWhen='(i-1)%10<1'" -p "Output.sampling.sampling.sampleWhen='(i-1)%10<1'"> /dev/null &
